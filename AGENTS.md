...
Every audit that rejects or rescopes a memory the extraction/classifier pipeline produced must fix the root cause, not just the symptom: turn that miss into a permanent regression case + few-shot example so the same mistake is caught automatically next time. Deleting the bad memory alone leaves the prompt that produced it unchanged, so it recurs. Route by the rejected memory's `source` (visible in `asaki_memory_list` output):
...
| `*:auto-extract`, `asaki_memory_extract` | server extraction | `test/fixtures/extraction-cases.json` | `SYSTEM_PROMPT` in `src/services/extraction.ts` | `npm run eval:extraction` (needs a live Worker + `ASAKI_MEMORY_BASE_URL`) |
...
Do it TDD-style so the improvement is provable: add the failing fixture case first, then update the few-shot copies, then run the eval to green. The fixture and the few-shot example must express the SAME contrast — a `classifier-cases.json` negative gets a matching `-> flag=false (reason)` line added to the classifier prompt in all three files at once so they never drift.

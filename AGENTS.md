...
- **Pi**: `package.json`'s `"pi": { "extensions": [...] }` field is Pi's own package manifest, declaring `integrations/pi/asaki-memory.ts` as the extension entry point (Pi looks for a `pi` key in `package.json`, same convention as an npm package). ALL machines — including this working copy — consume it as the published npm package `npm:@asaki14/pi-memory` (verified 2026-07-11: `~/.pi/agent/settings.json` packages list + `~/.pi/agent/npm/package.json`; an earlier local-path install was replaced at some point, so local edits do NOT take effect until published). Do NOT install from the git monorepo (Pi git sources clone the whole repo — no subpath support). Release flow is TAG-TRIGGERED CI — never `npm publish` locally: land the root `package.json` version bump on `main` through a PR (the pi package inherits that version), then push a `v*` tag (e.g. `v0.4.0`). `.github/workflows/publish.yml` fires on that tag, hard-fails unless the tag minus its `v` equals `package.json`'s version, runs `npm ci` + `npm run typecheck` + `npm run build:pi` (stages `dist/pi-package/` with just the single extension file + a minimal `pi`-key `package.json`), and publishes with `npm publish --access public` from `dist/pi-package/` via npm **Trusted Publishing** (OIDC, `id-token: write`) — no npm token, no 2FA OTP, no local credentials anywhere. Then on each machine `pi update npm:@asaki14/pi-memory`. Caveat: `pi update` honors `~/.pi/agent/npm/package-lock.json`, so it will NOT pull a newer patch that still satisfies the existing `^` range (it reports "up to date" and stays put, even after `npm cache clean`); to actually bump within-range, run `npm update @asaki14/pi-memory` inside `~/.pi/agent/npm`. Also note the npm read cache can lag right after publish — verify the new version landed with `npm view @asaki14/pi-memory version --prefer-online` (or curl the registry) rather than a plain `npm view`. Source of truth stays at `integrations/pi/asaki-memory.ts` — the build is a one-way sync, not a fork; never edit `dist/pi-package/` by hand.
...
- `src/services/memories.ts`: memory creation and search. `getMemoriesByIds()` backs `POST /v1/memories/get` — the batch read-by-id behind `asaki_memory_get` in all three clients: full content, no truncation, at most 20 ids, foreign ids reported as `missing` rather than returned, and it refreshes `last_accessed_at` because a read by id IS a retrieval (captain decision a, 2026-08-28).
...
- `scripts/eval-memory-get.ts`: offline unit coverage for `POST /v1/memories/get` against a fake D1 — the validation bounds (20-id cap, trimming/de-duplication), user scoping, request-order preservation, the `last_accessed_at` write and the `get` audit event. Run `npm run eval:memory-get` after touching that path.
...
- `integrations/claude-code/session-start.sh`: the SessionStart hook. Emits the standing-rule block, then the project digest, then the status banner, all from one memory-list response. Its top "library region" (the numeric env parser, the banner-line builder, and the two READ-side renderers for the classifier health ledger — `asaki_health_failing_field` / `asaki_health_idle_field`) sits above the `ASAKI_MEMORY_SESSION_START_LIB` guard so evals can `source` it without running a hook — same convention as `stop-extract.sh`; `integrations/claude-code/user-prompt.sh` has the same guard (`ASAKI_MEMORY_USER_PROMPT_LIB`) for its parser copies.
- `integrations/claude-code/stop-extract.sh`: the Stop hook. Its top "library region" (patterns plus `throttle_decision` / `outcome_for_status` / the `asaki_health_*` ledger writers) sits above the `ASAKI_MEMORY_STOP_EXTRACT_LIB` guard so evals can `source` it without running a hook — keep new hook side effects below that guard.
- Classifier health ledger: `$TMPDIR/asaki-memory-stop-extract/health.json` — the ONE state file in that dir that is deliberately **not** keyed by session id, because the failure mode it exists for (the bash-3.2 `${VAR: -N}` empty-delta regression) raises no error and is only visible as a count ACROSS sessions. `stop-extract.sh` writes it from the same advance/hold/giveup classification `outcome_for_status()` already produces (atomic temp+`mv`, `mkdir` lock, corrupt/missing tolerated); `session-start.sh` renders `!failing=N since=<date>` (alarm, ≥3) and `!idle=Nsessions` (hint only, ≥15) into the banner right after `classifier=`. Both are silent below threshold, so a healthy banner is unchanged. `npm run eval:health-ledger` covers the write side; `npm run eval:session-inject` covers the banner fields on both clients. The Pi extension keeps no on-disk classifier state and never fills these two fields — its `buildSessionBannerLine()` carries them only to stay one sync set with `asaki_banner_line()`. Field meanings are documented in `integrations/claude-code/README.md`.

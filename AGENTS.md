...
- `scripts/pi-trace-region.mjs`: loads the `// #region asaki-trace-builder` blocks of `integrations/pi/asaki-memory.ts` as a real module. The Pi extension must stay a single file (see "Integration install/update"), so an eval cannot import it directly; keep those region markers intact or the trace/signal evals fail loudly.

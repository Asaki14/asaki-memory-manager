...
The extension exposes `asaki_memory_search`, `asaki_memory_add`, `asaki_memory_list`, `asaki_memory_update`, `asaki_memory_delete`, `asaki_memory_review_create`, `asaki_memory_review_list`, `asaki_memory_review_resolve`, and the audit command under two names, `/memory` and `/memory-audit` (same handler: `status` checks connectivity, any other args are the audit focus).
...
Configuration carries over untouched: the extension resolves its agent directory as `PI_CODING_AGENT_DIR || ~/.pi/agent`, so under omp it keeps reading `~/.pi/agent/asaki-memory.json` and the same `ASAKI_MEMORY_*` environment as Pi. All the tools, the standing-rule block and the project digest behave identically.

The audit command does not keep its `/memory` name: omp has a built-in `/memory` and skips any extension command that collides with a built-in (it logs `Extension command 'memory' … conflicts with built-in commands. Skipping.` at startup). Run the audit as `/memory-audit` instead — same handler and arguments, and it works on Pi too. The alias ships in the release after `@asaki14/pi-memory` 0.4.1; until then a package install under omp has no audit command.
...
### Weekly consolidation report (local scheduled job, macOS)

A read-only weekly audit of the memory store: it fetches the pending review queue plus every ACTIVE global-scope memory, has a headless `claude -p` analyse them for duplicates, contradictions, staleness, wrong scope and noise, and writes a dry-run report labelling each finding KEEP / UPDATE(rescope) / MERGE / DELETE. It never calls a write endpoint, and the analysing model runs with no tools and no credentials — it only sees the two JSON payloads the script already fetched.

This used to be a cloud scheduled routine; it now runs locally, because the credentials live in local env files.

- Runner (source of truth): [`integrations/local/weekly-consolidation-report.sh`](integrations/local/weekly-consolidation-report.sh)
- Installer: [`integrations/local/install-weekly-launchagent.sh`](integrations/local/install-weekly-launchagent.sh)
- Schedule: **Monday 09:30 local**, LaunchAgent label `com.asaki.memory.weekly-report`
- Report output: `~/.local/state/asaki-memory/weekly-reports/YYYY-MM-DD.md` (plus a desktop notification); logs in `~/.local/state/asaki-memory/logs/`

```bash
# install or update (re-run after every change to the runner)
ASAKI_MEMORY_BASE_URL=https://your-worker.your-subdomain.workers.dev \
  bash integrations/local/install-weekly-launchagent.sh

# run it once, now
launchctl kickstart -p gui/$(id -u)/com.asaki.memory.weekly-report
```

**TCC note:** launchd cannot reliably execute anything under `~/Desktop` (macOS privacy protection). The installer therefore copies the runner to `~/.local/share/asaki-memory/weekly-runner/` and points the plist there, so the plist never references a checkout that may live under `~/Desktop`. The plist itself is machine-local and not tracked in this repo; it is regenerated by the installer and contains no credential — the runner reads `ASAKI_MEMORY_API_KEY` from the environment, falling back to the login shell (`fish -l`) when launchd starts it with an empty one, and never prints it.

This is a single-machine job by design: only the primary Mac runs it, and no copy is installed anywhere else.

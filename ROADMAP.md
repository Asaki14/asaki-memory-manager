...
**0.4.1（2026-09-10 采集侧修复，已验证）** — 引用请用 PR 号。

- 编排机项目归属漂移（#29）：`project-context.mjs` 的 allowlist 在编排机上把所有真实目标挡在门外、只留主机自身，且裸主机名被当作合法答案接受，两个缺陷叠加使跨项目候选被归入 firstmate（生产队列 9 行实证，时间全部晚于 v0.2.2，说明兜底从未生效）。改为 host 声明令牌 + allowlist 修正，Claude Code 与 Pi 两条集成路径同步。
- 三类分类器误抓固化为负例（#29）：一次性施工裁定、agent 自身系统提示词正文、环境仓库的配置现状复述；夹具 97→108，六份提示词副本同步补判别线。

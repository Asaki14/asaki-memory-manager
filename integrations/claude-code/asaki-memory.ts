/**
 * Asaki Memory MCP Server — Claude Code integration
 *
 * Exposes asaki_memory_search / add / list / update / delete via the MCP protocol (stdio).
 *
 * Setup (one-time):
 *   cd integrations/claude-code && npm install
 *
 ...
 *         "env": { "ASAKI_MEMORY_API_KEY": "your-admin-api-key" }
 *       }
 *     }
 *   }
 *
 * Add lifecycle hooks in ~/.claude/settings.json for auto-inject and memory nudge:
 *   SessionStart  → integrations/claude-code/session-start.sh
 *   UserPromptSubmit → integrations/claude-code/user-prompt.sh
 *
 ...
 *   ~/.claude/asaki-memory.json   { "apiKey": "...", "userId": "...", ... }
 */
...
function configFilePath(): string {
  return join(homedir(), ".claude", "asaki-memory.json");
}
...
async function apiRequest(
  ...
  if (!apiKey) {
    throw new Error(
      "ASAKI_MEMORY_API_KEY is not set. Add it to the MCP server env in ~/.claude/mcp.json or to ~/.claude/asaki-memory.json.",
    );
  }
...
server.tool(
  "asaki_memory_list",
  "List memories from Asaki personal memory with optional filters. Use during explicit memory audit or /memory command.",
  {

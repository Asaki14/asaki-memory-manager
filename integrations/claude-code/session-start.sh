...
# Hook: SessionStart
#
# Injects Asaki Memory status banner and instructions into Claude Code's
# session context. Fires on startup, resume, and compact.
#
...
\`ASAKI_MEMORY_API_KEY\` is not set. Add it to the MCP server env in \`~/.claude/mcp.json\`,
or create \`~/.claude/asaki-memory.json\` with \`{ "apiKey": "your-key" }\`.
BANNER
  exit 0
fi

# Fetch memory count (non-blocking, short timeout)
MEMORY_COUNT="?"
...
if [ "$SOURCE" = "startup" ]; then
  ...
  echo "Session ${SOURCE}. Search Asaki memory for recent session state and decisions to recover context. Run 2 parallel \`asaki_memory_search\` calls."
fi

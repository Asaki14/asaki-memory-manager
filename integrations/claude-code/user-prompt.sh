...
#    the keyword gate). Read-only (a memory search), so this never blocks on
#    anything more than the HTTP call itself.
#
...
SENSITIVE_RE='-----BEGIN [A-Z ]*PRIVATE KEY-----|Bearer [A-Za-z0-9._~+/=-]{16,}|(sk|sk-ant|sk-proj|ghp|gho|ghu|ghs|github_pat)_[A-Za-z0-9_=-]{16,}|AKIA[0-9A-Z]{16}|(api[_-]?key|token|secret|password|passwd|authorization)[[:space:]]*[:=]'
...
if [ "${ASAKI_MEMORY_AUTO_INJECT:-0}" = "1" ] \
  ...
  if ! printf '%s' "$PROMPT" | grep -qiE -- "$SENSITIVE_RE"; then
    if [ "${ASAKI_MEMORY_AUTO_INJECT_ALWAYS:-0}" = "1" ] || printf '%s' "$PROMPT" | grep -qiE "$MEMORY_NEEDED_RE"; then
      ...
      SEARCH_BODY=$(jq -cn --arg q "$PROMPT" --arg u "$ASAKI_USER" --arg p "$ASAKI_PROJECT" --argjson k "$AUTO_INJECT_TOP_K" \
        '{query: $q, user_id: $u, top_k: $k} + (if $p == "" then {} else {project_id: $p} end)')
        ...
        AUTO_CONTEXT=$(echo "$SEARCH_RESP" | jq -r --argjson minScore "$MIN_SCORE" --argjson maxChars "$MAX_INJECT_CHARS" '
          ...
          | if ($picked | length) == 0 then ""
              ...
              | ("Asaki memory search: injected " + ($picked | length | tostring) + "/" + ($all | length | tostring)
                  ...
                  + ($lines | join("\n"))) as $full
              | if ($full | length) > $maxChars then ($full[0:$maxChars] + "\n...(truncated, output budget reached)") else $full end
            end
        ' 2>/dev/null || echo "")
      fi
    fi

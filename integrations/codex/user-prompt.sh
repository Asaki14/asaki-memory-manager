...
#
# Injects a memory precheck instruction into Codex's context on each user message.
set -uo pipefail
...
if echo "$PROMPT" | grep -qiE "$RESUME_RE"; then
  ...
  _CTX="This turn may need durable memory. Call asaki_memory_search only if the answer depends on remembered preferences, prior decisions, conventions, or past project facts."
else

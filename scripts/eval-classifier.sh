...
for i in $(seq 0 $((CASE_COUNT - 1))); do
  ...
  RESP=$(claude -p --safe-mode --tools "" --model "$CLASSIFIER_MODEL" --json-schema "$SCHEMA" "$PROMPT" 2>/dev/null)
